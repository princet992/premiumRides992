...
export const rideSchema = z.object({
  ...
  availableSeats: z.number({ invalid_type_error: "Available seats must be a number" }).min(1, "Min 1 seat is required"),
  pricePerSeat: z.number({ invalid_type_error: "Price must be a number" }).min(0, "Price must be at least 0"),
